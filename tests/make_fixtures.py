"""Writes the fixtures the app's unit tests compare against.

    python tests/make_fixtures.py

`app/lib/core/rule.dart` and `indicators.dart` are ports of `rule.py` and `indicators.py`.
These files hold Python's answers so the Dart tests (run in every app build) catch any drift.
"""
import json
import math
import random
...
OUT = HERE.parent / 'app/test/fixtures'
CONFIGS = [
    {},
    dict(stoch='fast'),
